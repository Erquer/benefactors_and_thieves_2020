...
//choose to destroy flowerpot or toilet
//0 -> flowerpot
//1 -> toilet
bool Thieve::flowerpotOrToilet() {
    //create fixed flowerpots counter
...
int Thieve::findItemToDestroy() {
    ...
    if (choice == FLOWERPOT){
        ...
        for(int flowerpotID = 0; flowerpotID < this->flowerpots.size(); flowerpotID++){
            ...
            //if flowerpot has no requests choose it as an item to destroy
            if(noRequests){
                return flowerpotID;
            }
    ...
    //toilet choosen
        ...
        for(int toiletID = 0; toiletID < this->toilets.size(); toiletID++){
            ...
            //if toilet has no requests choose it as an item to destroy
            if(noRequests){
                return toiletID;
            }
